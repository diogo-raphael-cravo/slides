...
const Text: React.FC<{
...
}> = function ({ id, text, position, onChangePosition, onDragStart, onTextChanged }) {
    const [dimensions, setDimensions] = useState<{ w: number, h: number }>({ w: 200, h: 100 });
    ...
    function getGeometry(style: Record<string, string | number>) {
        return <textarea className='non-focusable' style={{
            ...style,
            fontSize: 20,
            border: '0px',
            backgroundColor: '#fff',
            padding: 0,
            resize: 'none',
        }} value={text.content} onChange={(e) => onTextChanged(e.target.value)}></textarea>
    }
